# RECOVERY crossover from CS to AVR simulation
...
# Given median = 700 and IQR = 1469 - 277, we can estimate the shape and scale parameters
...
set.seed(123)
event_times <- rgamma(53, shape = shape_gamma, scale = scale_gamma)

# Step 3: Add 19 censored patients
# Specific censoring times you provided:
specific_censor_times <- c(127, 238, 325, 661, 1334)

# Remaining censored at day 2930
remaining_censored <- rep(2930, 14)  # 19 - 5 = 14
...
# Step 5a: Count how many had events before 365 days
num_events_before_365 <- dat %>% filter(status == 1, time <= 365) %>% nrow()
cat("Number of events before day 365:", num_events_before_365, "\n")
