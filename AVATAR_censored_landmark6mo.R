...
#------------------------------------------------------------
# 6. Censored at 1 year - 6 months for AVATAR data
#------------------------------------------------------------
ipd_censored <- ipd_all
ipd_censored$time <- pmin(ipd_censored$time, 6)  # Censor at 6 months
ipd_censored$event[ipd_censored$time == 6] <- 0
cox_model_censored <- coxph(Surv(time, event) ~ group, data = ipd_censored)
print(summary(cox_model_censored))

#------------------------------------------------------------
# 7. Landmark analysis starting from 0.5 year - 6 months for AVATAR data
#------------------------------------------------------------
ipd_landmark <- ipd_all %>% filter(time > 6)
ipd_landmark$time <- ipd_landmark$time - 6  # Reset time from 12 months
