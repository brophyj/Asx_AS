...
# original data from EARLY_TAVR.csv and EARLY_Ctl.csv
# try new data from EARLY_TAVR1.csv and EARLY_Ctl1.csv
#------------------------------------------------------------
data_int <- read_csv("Input/EARLY_TAVR1.csv", col_names = c("time", "incidence"),
                     show_col_types = FALSE) %>% 
  mutate(across(everything(), as.numeric)) %>% 
  na.omit()

data_clinical <- read_csv("Input/EARLY_Ctl1.csv", col_names = c("time", "incidence"),
                          show_col_types = FALSE) %>% 
  mutate(across(everything(), as.numeric)) %>% 
  na.omit()
...
at_risk_int = c(455, 390, 363, 285, 141, 103)
at_risk_clin = c(446, 305, 266, 187, 117, 46)
#------------------------------------------------------------
...
#------------------------------------------------------------
# 6. Censored at 6 months
#------------------------------------------------------------
ipd_censored <- ipd_all
ipd_censored$time <- pmin(ipd_censored$time, 0.5)  # Censor at 6 months
ipd_censored$event[ipd_censored$time == 0.5] <- 0
cox_model_censored <- coxph(Surv(time, event) ~ group, data = ipd_censored)
print(summary(cox_model_censored))

#------------------------------------------------------------
# 7. Landmark analysis starting from 6
#------------------------------------------------------------
ipd_landmark <- ipd_all %>% filter(time > 0.5)  # Start from 6 months
ipd_landmark$time <- ipd_landmark$time - 0.5  # Reset time from from 6 months
