...
#------------------------------------------------------------
data_int <- read_csv("Input/AVATAR_Sx.csv") %>% 
  rename(time = 1, incidence = 2) %>% 
  mutate(across(everything(), as.numeric)) %>% 
  na.omit()

data_clinical <- read_csv("Input/AVATAR_Ctl.csv") %>% 
  rename(time = 1, incidence = 2) %>% 
  mutate(across(everything(), as.numeric)) %>% 
  na.omit()
...
at_risk_int = c(78, 69, 67, 48, 13)
at_risk_clin = c(79, 61, 52, 29, 11)
#------------------------------------------------------------
...
#------------------------------------------------------------
# 6. Censored at 1 year - 12 months for AVATAR data
#------------------------------------------------------------
ipd_censored <- ipd_all
ipd_censored$time <- pmin(ipd_censored$time, 12)  # Censor at 12 months
ipd_censored$event[ipd_censored$time == 12] <- 0
cox_model_censored <- coxph(Surv(time, event) ~ group, data = ipd_censored)
print(summary(cox_model_censored))

#------------------------------------------------------------
# 7. Landmark analysis starting from 1 year - 12 months for AVATAR data
#------------------------------------------------------------
ipd_landmark <- ipd_all %>% filter(time > 12)
ipd_landmark$time <- ipd_landmark$time - 12  # Reset time from 12 months
