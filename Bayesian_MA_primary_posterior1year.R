...
dat <- tibble(
  study = c("EVOLVED", "AVATAR", "EARLY_TAVR"),
  y = c(1.20, 0.46, 0.32),
  lower = c(0.52, 0.18, 0.23),
  upper = c(2.78, 1.22, 0.44)
)  %>% 
  mutate(
    y_log = log(y),
...
p <- ggplot() +
  ...
  labs(
    title = "Supplemental Figure 3 - Bayesian (Posterior) Estimates for Primary Outcome \nfrom IPD Censored at 1 year ",
    x = "Hazard Ratio (log scale)", y = NULL,
    caption = "One year data not avalilable from RECOVERY trial"
  ) +
  theme_minimal(base_size = 13) +
  theme(plot.title = element_text(face = "bold"))

print(p)
ggsave("Output/Bayesian_MA_primary_posterior1yr.png", width = 10, height = 6, dpi = 300)
