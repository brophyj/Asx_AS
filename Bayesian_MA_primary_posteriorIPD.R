...
dat <- tibble(
  ...
  y     = c(0.05, 0.71, 0.51, 0.57),
  lower = c(0.01, 0.43, 0.27, 0.45),
  upper = c(0.40, 1.18, 0.94, 0.70)
) |>
  mutate(
    y_log = log(y),
...
p <- ggplot() +
  ...
  labs(
    title = "Bayesian (Posterior) Estimates of Primary Outcome from IPD",
...
print(p)
ggsave("Output/Bayesian_MA_primary_posterior_IPD.png", width = 10, height = 6, dpi = 300)
