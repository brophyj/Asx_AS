...
#------------------------------------------------------------
data_int <- read.csv("Input/AVATAR_Sx.csv", header = FALSE)       # time, incidence (%)
data_clinical <- read.csv("Input/AVATAR_Ctl.csv", header = FALSE) # time, incidence (%)
...
at_risk_int  <- c(78, 69, 67, 48, 13)       # Intervention
at_risk_clin <- c(79, 61, 52, 29, 11)       # Clinical
...
ggsurvplot(
  fit_overall, data = ipd_all,
  risk.table = TRUE, conf.int = TRUE,
  ggtheme = theme_minimal(), legend.title = "Group"
)

#------------------------------------------------------------
# 6. Restrict analysis to 12 months by censoring after 12
#------------------------------------------------------------
ipd_censor12 <- ipd_all %>%
  mutate(
    # If time > 12, censor at 12
    time_12 = pmin(time, 12),
    event_12 = if_else(time <= 12, event, 0)
  )

cox_12 <- coxph(Surv(time_12, event_12) ~ group, data = ipd_censor12)
summary(cox_12)

# Print HR + CI
hr_12 <- exp(coef(cox_12))
ci_12 <- exp(confint(cox_12))
cat("HR at <= 12 months =", hr_12, "\n95% CI =", ci_12[1], "to", ci_12[2], "\n")

# Plot KM up to 12 months, still starts with 79 vs 78
fit_12 <- survfit(Surv(time_12, event_12) ~ group, data = ipd_censor12)
ggsurvplot(
  fit_12, data = ipd_censor12,
  xlim = c(0, 12),
  risk.table = TRUE, conf.int = TRUE,
  ggtheme = theme_minimal(), legend.title = "Group"
)
