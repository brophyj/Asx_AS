...
dat <- tibble(
  ...
  y = c(0.33, 1.22, 0.44, 0.93),
  lower = c(0.12, 0.59, 0.23, 0.60),
  upper = c(0.90, 2.51, 0.85, 1.44)
) %>%
  mutate(
    y_log = log(y),
...
p <- ggplot() +
  ...
  labs(
    title = "Figure 4 - Bayesian Estimates of Total Mortality from Published Data",
...
print(p)
ggsave("Output/Bayesian_MA_mortality.png", width = 10, height = 6, dpi = 300)
